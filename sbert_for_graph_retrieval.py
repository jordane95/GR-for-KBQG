...
refs_train = [[train_set[i]['id'] for i in idxs[1:]] for idxs in I_train] # elimating itself for train set
refs_dev = [[train_set[i]['id'] for i in idxs] for idxs in I_dev]
refs_test = [[train_set[i]['id'] for i in idxs] for idxs in I_test]
...
with open(os.path.join(args.data_path, 'sbert_train_for_graph_retrieval.json'), 'w') as fpw:
    json.dump(new_train_set, fpw)
with open(os.path.join(args.data_path, 'sbert_dev_for_graph_retrieval.json'), 'w') as fpw:
    json.dump(new_dev_set, fpw)
with open(os.path.join(args.data_path, 'sbert_test_for_graph_retrieval.json'), 'w') as fpw:
